...
Este proyecto fue mi primera toma de contacto seria con las señales de UNIX. El objetivo es el siguiente: crear un programa de comunicación Cliente-Servidor, pero con una restricción importante, solo podía usar dos señales para transmitir toda la información (`SIGUSR1` y `SIGUSR2`).
...
2.  **Inicia el servidor:**
    ...
    ./server
    ```
    (Verás que imprime su PID en la terminal. Cópialo).

3.  **Lanza el cliente (en otra terminal):**
    Pásale el PID del servidor y el mensaje que quieras enviar.
    ```bash
